...
TEST(Unicode, ParseInvalidChars) {
	...
	uchar *result1 = utf8_parse((const char *)utf8_with_invalid_bytes, &error);
	ASSERT_NE(result1, nullptr);
	EXPECT_EQ(error, true);
	EXPECT_EQ(
		memcmp(result1, uc_with_invalid_bytes, sizeof(uc_with_invalid_bytes)),
		0);
	free(result1);

	uchar *result2 = utf8_parse((const char *)utf8_multibyte_errors, &error);
	ASSERT_NE(result2, nullptr);
	EXPECT_EQ(error, true);
	EXPECT_EQ(
		memcmp(result2, uc_multibyte_errors, sizeof(uc_multibyte_errors)), 0);
	free(result2);
}
